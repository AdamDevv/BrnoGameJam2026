...
{
    public class BaseSafeSignal : IBaseSignal
    {
        public static event Action<Exception> OnExceptionEvent;
        ...
        public void AddListener(Action<IBaseSignal, object[]> callback) { BaseListener = AddUnique(BaseListener, callback); }

        /// <summary>
        /// Adds a listener which will be removed immediately after the Signal fires.
        /// </summary>
        /// <param name="callback">The method to be called when Dispatch fires.</param>
        public void AddOnce(Action<IBaseSignal, object[]> callback) { OnceBaseListener = AddUnique(OnceBaseListener, callback); }
