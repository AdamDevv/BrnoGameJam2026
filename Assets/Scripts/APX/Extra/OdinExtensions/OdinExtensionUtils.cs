...
namespace APX.Extra.OdinExtensions
{
    public static class OdinExtensionUtils
    {
        ...
        public static void DrawSubtypeDropDownOrCall(Rect dropdownPosition, Type type, Action<Type> onSelectAction)
        {
            ...
            var assignableTypes = ReflectionHelper.GetAssignableSubtypes(type);
        ...
        public static ValueDropdownList<Type> GetAssignableSubtypes(Type targetType)
        {
            if (_cachedAssignableTypes == null)
                _cachedAssignableTypes = new Dictionary<Type, ValueDropdownList<Type>>();

            if (!_cachedAssignableTypes.TryGetValue(targetType, out ValueDropdownList<Type> result))
            {
                result = GetAssignableSubtypesInternal(targetType);
                _cachedAssignableTypes.Add(targetType, result);
            }

            return result;
        }

        private static ValueDropdownList<Type> GetAssignableSubtypesInternal(Type targetType)
        {
            var subtypes = ReflectionHelper.GetAssignableSubtypes(targetType);
            var result = new ValueDropdownList<Type>();
            foreach (var subtype in subtypes)
            {
                result.Add(subtype.Name, subtype);
            }
            return result;
        }
