...
namespace APX.Extra.Misc
{
    public static class RandomFromDistribution
    {
            ...
            ConfidenceLevel_e confidence_level_cutoff /*, float confidence_level_cutoff*/)
        {
            var oldMin = min;
            var oldMax = max;
            min = Mathf.Min(oldMin, oldMax);
            max = Mathf.Max(oldMin, oldMax);
        ...
        public static float RandomRangeSlope(float min, float max, float skew, Direction_e direction) { return min + RandomFromSlopedDistribution(skew, direction) * (max - min); }
        ...
        public static float RandomRangeExponential(float min, float max, float exponent, Direction_e direction) { return min + RandomFromExponentialDistribution(exponent, direction) * (max - min); }
        ...
        private static float ExponentialRightInverse(float y, float exponent) { return Mathf.Pow(y, 1.0f / exponent); }
        ...
        public static int RandomChoiceFollowingDistribution(List<float> probabilities)
        {
            ...
            // Choose from CDF:
            float cdf_value = Range(0.0f, cdf[probabilities.Count - 1]);
