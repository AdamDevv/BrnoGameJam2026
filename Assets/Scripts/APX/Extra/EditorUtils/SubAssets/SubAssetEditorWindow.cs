...
namespace APX.Extra.EditorUtils.SubAssets
{
    public class SubAssetEditorWindow : OdinEditorWindow, IHasCustomMenu
    {
        ...
        [ListDrawerSettings(ShowFoldout = false, IsReadOnly = true, OnTitleBarGUI = nameof(OnSubAssetsTitleBarGUI))]
        ...
        private void OnSubAssetsTitleBarGUI()
        {
            if (SirenixEditorGUI.ToolbarButton("Add From References")) 
            {
                AddFromReferences();
            }
        }
        
        private void AddFromReferences()
        {
            var referencedAssets = EditorAssetUtils.GetAllReferencedObjectsByAssetHierarchy(_currentAsset);

            var selector = new GenericSelector<Object>("References", true, x => x.name, referencedAssets);
            ...
            selector.SelectionConfirmed += selection =>
            {
                if (selection.Any())
                {
                    SubAssetUtils.MoveAssets(selection, _currentAsset);
                    RefreshCurrent();
                }
            };
        ...
        private void OnSubAssetsChanged(object sender, NotifyCollectionChangedEventArgs notifyCollectionChangedEventArgs)
        {
            ...
            SubAssetUtils.MoveAssets(newItems, _currentAsset);
            RefreshCurrent();
        }
        ...
        private Object CustomSubAssetValueDrawer(Object value, GUIContent label, Func<GUIContent, bool> callNextDrawer)
        {
            ...
            {
                SubAssetUtils.ExportSubAssetToCurrentFolder(value);
            }
            
            if (SirenixEditorGUI.IconButton(EditorIcons.X, 18, 18, "Delete"))
            {
                SubAssetUtils.DeleteSubAsset(value);
            }
